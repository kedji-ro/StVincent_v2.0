...
if (isset($_POST['u_login'])) {
    ...
    $sql = mysqli_query($conn, "SELECT _id, _username, _password, _fullname, _email, _mobile, _role, _volunteer, _address FROM `tb_useracct` WHERE _username = '" . $user . "' AND _password = MD5('" . $pass . "') AND _active = 0 AND _role ='user';");
    $sql_active = mysqli_query($conn, "SELECT _id, _username, _password, _fullname, _email, _mobile, _role, _volunteer, _address FROM `tb_useracct` WHERE _username = '" . $user . "' AND _password = MD5('" . $pass . "') AND _active = 1 AND _role ='user';");
...
if (isset($_POST['a_login'])) {
    ...
    $sql = mysqli_query($conn, "SELECT _id, _username, _password, _fullname, _email, _mobile, _role, _volunteer, _address FROM `tb_useracct` WHERE _username = '" . $user . "' AND _password = MD5('" . $pass . "') AND _active = 0 AND _role ='admin';");
    $sql_active = mysqli_query($conn, "SELECT _id, _username, _password, _fullname, _email, _mobile, _role, _volunteer, _address FROM `tb_useracct` WHERE _username = '" . $user . "' AND _password = MD5('" . $pass . "') AND _active = 1 AND _role ='admin';");
    ...
    if ($row_cnt_active > 0) { // Has Account
        ...
        header("location: http://localhost:8080/GitHub/StVincent_v2.0/admin/?1");
    }
